...
import tagcloud.backup.CrawlCallable_old;
	...
	public List<String> crawl(final String startURL) {
		...
		while (!urlsToVisit.isEmpty()) {
			for (String url : urlsToVisit) {
				taskQueue.submit(new CrawlCallable_old(url));
				urlsCrawled.add(url);
			...
			try {
				...
				while (entries.hasNext()) {
				  ...
				  for(String url: links) {
					  if(!urlsCrawled.contains(url) && !urlsToVisit.contains(url) && url.startsWith(startURL)) {
						  urlsToVisit.add(url);
					  }
				  }
